...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        W1, b1 = self.params['W1'], self.params['b1']  # Извлечение весов и смещений первого слоя
        W2, b2 = self.params['W2'], self.params['b2']  # Извлечение весов и смещений второго слоя

        # Прямой проход через первый слой (affine) и применение нелинейности ReLU
        # hidden - активации после ReLU, cache_hidden - кэш для обратного прохода
        hidden, cache_hidden = affine_relu_forward(X, W1, b1)

        # Прямой проход через второй слой (affine) без применения нелинейности
        # scores - выходные оценки классов, cache_out - кэш для обратного прохода
        ...
        # Расчет потерь через функцию softmax и добавление L2 регуляризации
        loss, dSoftmax = softmax_loss(scores, y)
        reg_loss = 0.5 * self.reg * (np.sum(W1**2) + np.sum(W2**2))
        loss += reg_loss

        # Обратное распространение через второй слой (affine)
        dx2, dw2, db2 = affine_backward(dSoftmax, cache_out)
        # Добавляем регуляризацию к градиенту весов
        dw2 += self.reg * W2

        # Обратное распространение через первый слой (affine + relu)
        dx1, dw1, db1 = affine_relu_backward(dx2, cache_hidden)
        # Добавляем регуляризацию к градиенту весов
        dw1 += self.reg * W1

        # Сохраняем градиенты в словарь grads
...
class FullyConnectedNet(object):
    ...
    def __init__(
        ...
        # Итерируем через все слои нейронной сети
    ...
    def loss(self, X, y=None):
        ...
        for layer_index in range(1, self.num_layers):
            ...
            if self.normalization == "batchnorm":
                ...
                bn_output, bn_cache = batchnorm_forward(fc_output, self.params[gamma_key], self.params[beta_key], self.bn_params[layer_index - 1])
        ...
        # Вычисляем потери и градиент для softmax.
